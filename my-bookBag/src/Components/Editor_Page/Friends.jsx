import React, { Component } from 'react';
import './Friends.css';
import {SettingsPane, SettingsPage, SettingsContent, SettingsMenu} from 'react-settings-pane';


class Friends extends Component {
  render(){

 const menu = [
   {
     title: 'General',          // Title that is displayed as text in the menu
     url: '/settings/general'  // Identifier (url-slug)
   }
 ];

    return(
      <div class="friend">
      <h5 class="fr">Friends</h5>
      <div class="line1"></div>

      <table class="table">

  <tbody>
    <tr>
      <th scope="row"><svg id='Capa_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 53 53'>
     <path d='M18.613,41.552l-7.907,4.313c-0.464,0.253-0.881,0.564-1.269,0.903C14.047,50.655,19.998,53,26.5,53	c6.454,0,12.367-2.31,16.964-6.144c-0.424-0.358-0.884-0.68-1.394-0.934l-8.467-4.233c-1.094-0.547-1.785-1.665-1.785-2.888v-3.322	c0.238-0.271,0.51-0.619,0.801-1.03c1.154-1.63,2.027-3.423,2.632-5.304c1.086-0.335,1.886-1.338,1.886-2.53v-3.546	c0-0.78-0.347-1.477-0.886-1.965v-5.126c0,0,1.053-7.977-9.75-7.977s-9.75,7.977-9.75,7.977v5.126	c-0.54,0.488-0.886,1.185-0.886,1.965v3.546c0,0.934,0.491,1.756,1.226,2.231c0.886,3.857,3.206,6.633,3.206,6.633v3.24	C20.296,39.899,19.65,40.986,18.613,41.552z'
     fill='#e7eced' />
     <path d='M26.953,0.004C12.32-0.246,0.254,11.414,0.004,26.047C-0.138,34.344,3.56,41.801,9.448,46.76 c0.385-0.336,0.798-0.644,1.257-0.894l7.907-4.313c1.037-0.566,1.683-1.653,1.683-2.835v-3.24c0,0-2.321-2.776-3.206-6.633 c-0.734-0.475-1.226-1.296-1.226-2.231v-3.546c0-0.78,0.347-1.477,0.886-1.965v-5.126c0,0-1.053-7.977,9.75-7.977 s9.75,7.977,9.75,7.977v5.126c0.54,0.488,0.886,1.185,0.886,1.965v3.546c0,1.192-0.8,2.195-1.886,2.53 c-0.605,1.881-1.478,3.674-2.632,5.304c-0.291,0.411-0.563,0.759-0.801,1.03V38.8c0,1.223,0.691,2.342,1.785,2.888l8.467,4.233 c0.508,0.254,0.967,0.575,1.39,0.932c5.71-4.762,9.399-11.882,9.536-19.9C53.246,12.32,41.587,0.254,26.953,0.004z'
     fill='#556080' />
     </svg></th>
      <td class="row1">Bob Marley</td>
      <td class="row1"><svg class="reddot" height="15" width="15">
  <circle cx="10" cy="10" r="5" fill="#53c721" />
</svg></td>

    </tr>
    <tr>
      <th scope="row"><svg id='Capa_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 53 53'>
    <path d='M11.487,44.641l9.113-3.908c0.494-0.247,0.896-1.045,1.179-1.614c0.255-0.514-0.126-1.119-0.7-1.119	h-6.445c0,0-2.33-0.228-4.127-1c-1.053-0.452-1.415-1.049-0.757-1.986c1.925-2.74,5.904-9.192,6.033-15.855	c0,0,0.221-10.257,10.34-10.34c5.903,0.049,8.691,3.523,9.99,6.41c0.724,1.611,1.035,3.367,1.151,5.129	c0.42,6.351,3.654,12.16,5.161,14.642c0.511,0.842,0.264,1.963-0.581,2.469C40.458,38.298,38.761,38,38.761,38h-6.488	c-0.634,0-0.914,0.886-0.416,1.279c0.406,0.32,0.793,0.609,1.002,0.723l7.58,4.587c1.094,0.597,1.967,1.483,2.561,2.529	C38.451,50.795,32.664,53,26.359,53c-6.574,0-12.585-2.399-17.216-6.363C9.748,45.812,10.537,45.116,11.487,44.641z'
    fill='#e7eced' />
    <path d='M26.5,0C11.864,0,0,11.865,0,26.5c0,8.022,3.57,15.206,9.202,20.066 c0.598-0.793,1.364-1.464,2.286-1.925l9.113-3.908c0.494-0.247,0.896-1.045,1.179-1.614c0.255-0.514-0.126-1.119-0.7-1.119h-6.445 c0,0-2.33-0.228-4.127-1c-1.053-0.452-1.415-1.049-0.757-1.986c1.925-2.74,5.904-9.192,6.033-15.855c0,0,0.221-10.257,10.34-10.34 c5.903,0.049,8.691,3.523,9.99,6.41c0.724,1.611,1.035,3.367,1.151,5.129c0.42,6.351,3.654,12.16,5.161,14.642 c0.511,0.842,0.264,1.963-0.581,2.469C40.458,38.298,38.761,38,38.761,38h-6.488c-0.634,0-0.914,0.886-0.416,1.279 c0.406,0.32,0.793,0.609,1.002,0.723l7.58,4.587c1.086,0.592,1.952,1.469,2.545,2.505c0.026,0.025,0.055,0.048,0.08,0.075 C49.117,42.313,53,34.864,53,26.5C53,11.865,41.136,0,26.5,0z'
    fill='#556080' />
</svg></th>
      <td class="row1">Jen Aniston</td>
      <td class="row1"><svg class="reddot" height="15" width="15">
  <circle cx="10" cy="10" r="5" fill="#53c721" />
</svg></td>

    </tr>
    <tr>
      <th scope="row"><svg id='Capa_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 53 53'>
    <path d='M11.487,44.641l9.113-3.908c0.494-0.247,0.896-1.045,1.179-1.614c0.255-0.514-0.126-1.119-0.7-1.119	h-6.445c0,0-2.33-0.228-4.127-1c-1.053-0.452-1.415-1.049-0.757-1.986c1.925-2.74,5.904-9.192,6.033-15.855	c0,0,0.221-10.257,10.34-10.34c5.903,0.049,8.691,3.523,9.99,6.41c0.724,1.611,1.035,3.367,1.151,5.129	c0.42,6.351,3.654,12.16,5.161,14.642c0.511,0.842,0.264,1.963-0.581,2.469C40.458,38.298,38.761,38,38.761,38h-6.488	c-0.634,0-0.914,0.886-0.416,1.279c0.406,0.32,0.793,0.609,1.002,0.723l7.58,4.587c1.094,0.597,1.967,1.483,2.561,2.529	C38.451,50.795,32.664,53,26.359,53c-6.574,0-12.585-2.399-17.216-6.363C9.748,45.812,10.537,45.116,11.487,44.641z'
    fill='#e7eced' />
    <path d='M26.5,0C11.864,0,0,11.865,0,26.5c0,8.022,3.57,15.206,9.202,20.066 c0.598-0.793,1.364-1.464,2.286-1.925l9.113-3.908c0.494-0.247,0.896-1.045,1.179-1.614c0.255-0.514-0.126-1.119-0.7-1.119h-6.445 c0,0-2.33-0.228-4.127-1c-1.053-0.452-1.415-1.049-0.757-1.986c1.925-2.74,5.904-9.192,6.033-15.855c0,0,0.221-10.257,10.34-10.34 c5.903,0.049,8.691,3.523,9.99,6.41c0.724,1.611,1.035,3.367,1.151,5.129c0.42,6.351,3.654,12.16,5.161,14.642 c0.511,0.842,0.264,1.963-0.581,2.469C40.458,38.298,38.761,38,38.761,38h-6.488c-0.634,0-0.914,0.886-0.416,1.279 c0.406,0.32,0.793,0.609,1.002,0.723l7.58,4.587c1.086,0.592,1.952,1.469,2.545,2.505c0.026,0.025,0.055,0.048,0.08,0.075 C49.117,42.313,53,34.864,53,26.5C53,11.865,41.136,0,26.5,0z'
    fill='#556080' />
</svg></th>
      <td class="row1">Mrs. Claus</td>
      <td class="row1"><svg class="reddot" height="15" width="15">
  <circle cx="10" cy="10" r="5" fill="red" />
</svg></td>

    </tr>

    <tr>
      <th scope="row"><svg id='Capa_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 53 53'>
     <path d='M18.613,41.552l-7.907,4.313c-0.464,0.253-0.881,0.564-1.269,0.903C14.047,50.655,19.998,53,26.5,53	c6.454,0,12.367-2.31,16.964-6.144c-0.424-0.358-0.884-0.68-1.394-0.934l-8.467-4.233c-1.094-0.547-1.785-1.665-1.785-2.888v-3.322	c0.238-0.271,0.51-0.619,0.801-1.03c1.154-1.63,2.027-3.423,2.632-5.304c1.086-0.335,1.886-1.338,1.886-2.53v-3.546	c0-0.78-0.347-1.477-0.886-1.965v-5.126c0,0,1.053-7.977-9.75-7.977s-9.75,7.977-9.75,7.977v5.126	c-0.54,0.488-0.886,1.185-0.886,1.965v3.546c0,0.934,0.491,1.756,1.226,2.231c0.886,3.857,3.206,6.633,3.206,6.633v3.24	C20.296,39.899,19.65,40.986,18.613,41.552z'
     fill='#e7eced' />
     <path d='M26.953,0.004C12.32-0.246,0.254,11.414,0.004,26.047C-0.138,34.344,3.56,41.801,9.448,46.76 c0.385-0.336,0.798-0.644,1.257-0.894l7.907-4.313c1.037-0.566,1.683-1.653,1.683-2.835v-3.24c0,0-2.321-2.776-3.206-6.633 c-0.734-0.475-1.226-1.296-1.226-2.231v-3.546c0-0.78,0.347-1.477,0.886-1.965v-5.126c0,0-1.053-7.977,9.75-7.977 s9.75,7.977,9.75,7.977v5.126c0.54,0.488,0.886,1.185,0.886,1.965v3.546c0,1.192-0.8,2.195-1.886,2.53 c-0.605,1.881-1.478,3.674-2.632,5.304c-0.291,0.411-0.563,0.759-0.801,1.03V38.8c0,1.223,0.691,2.342,1.785,2.888l8.467,4.233 c0.508,0.254,0.967,0.575,1.39,0.932c5.71-4.762,9.399-11.882,9.536-19.9C53.246,12.32,41.587,0.254,26.953,0.004z'
     fill='#556080' />
     </svg></th>
      <td class="row1">Larry the Bird</td>
      <td class="row1"><svg class="reddot" height="15" width="15">
  <circle cx="10" cy="10" r="5" fill="red" />
</svg></td>

    </tr>
    <tr>
      <th scope="row"><svg id='Capa_1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 59.27 59.27'>
    <path d='M53.566,44.427l-9.552-4.776C42.78,39.034,42,37.772,42,36.392v-3.748 c0.268-0.305,0.576-0.698,0.904-1.162c1.302-1.838,2.286-3.861,2.969-5.984C47.098,25.121,48,23.989,48,22.644v-4 c0-0.88-0.391-1.667-1-2.217v-5.783c0,0,1.187-9-11-9c-12.188,0-11,9-11,9v5.783c-0.609,0.55-1,1.337-1,2.217v4 c0,1.054,0.554,1.981,1.383,2.517C26.382,29.513,29,32.644,29,32.644v3.655c0,1.333-0.728,2.56-1.899,3.198l-8.921,4.866 C15.603,45.77,14,48.47,14,51.406v3.239h44v-3.043C58,48.563,56.283,45.786,53.566,44.427z'
    fill='#8697cb' />
    <path d='M18.18,44.364l5.946-3.243c-0.034-0.033-0.005-0.043,0.065-0.036l2.91-1.587 C28.272,38.859,29,37.633,29,36.299v-3.655c0,0-1.062-1.275-2.092-3.323h0c0-0.001-0.001-0.002-0.001-0.003 c-0.135-0.268-0.268-0.551-0.399-0.844c-0.018-0.041-0.036-0.08-0.054-0.121c-0.133-0.303-0.263-0.616-0.386-0.944 c-0.008-0.021-0.015-0.044-0.023-0.065c-0.108-0.29-0.209-0.589-0.306-0.896c-0.026-0.084-0.052-0.167-0.077-0.251 c-0.101-0.338-0.196-0.682-0.278-1.038C24.554,24.626,24,23.698,24,22.644v-4c0-0.88,0.391-1.667,1-2.217V10.78 c-1.413-1.097-3.603-2.136-7-2.136c-9.563,0-10,8-10,8v4.995c-0.526,0.475-1,1.154-1,1.914v3.455c0,0.911,0.479,1.711,1.194,2.174 c0.863,3.758,3.76,6.463,3.76,6.463v3.157c0,1.151-0.629,2.211-1.64,2.762L3.61,45.766C1.385,46.98,0,49.312,0,51.847v2.797h14 v-3.239C14,48.47,15.603,45.77,18.18,44.364z'
    fill='#556080' />
    <polygon points='37.836,48.133 37.828,48.14 35.84,55.43 40.487,50.783'
    fill='#eddcc7' />
    <path d='M57.086,34.184l-1.247-1.247c-0.775-0.775-2.032-0.775-2.807,0l-3.582,3.582L52.1,39.17 L57.086,34.184z'
    fill='#d75a4a' />
    <rect x='43.094' y='35.439' transform='rotate(45.001 44.969 43.65)' width='3.749'
    height='16.424' fill='#f29c21' />
    <polygon points='43.485,53.797 43.493,53.79 40.487,50.783 35.84,55.43 35.707,55.918'
    fill='#d6c4b1' />
    <path d='M55.107,42.176l3.582-3.582c0.775-0.775,0.775-2.032,0-2.807l-1.602-1.602L52.1,39.17 L55.107,42.176z'
    fill='#a34740' />
    <rect x='45.671' y='38.267' transform='rotate(-134.999 47.796 46.48)'
    width='4.251' height='16.424' fill='#e18c25' />
    <path d='M35,57.626c-0.256,0-0.512-0.098-0.707-0.293c-0.391-0.391-0.391-1.023,0-1.414l2.207-2.207 c0.391-0.391,1.023-0.391,1.414,0s0.391,1.023,0,1.414l-2.207,2.207C35.512,57.528,35.256,57.626,35,57.626z'
    fill='#5e5e5e' />
</svg></th>
      <td class="row1">Chemistry Group</td>
      <td class="row1"><svg class="reddot" height="15" width="15">
  <circle cx="10" cy="10" r="5" fill="red" />
</svg></td>

    </tr>

    <tr>
      <th scope="row"></th>
      <td class="row1">New Group</td>
      <td class="row1"></td>

    </tr>
  </tbody>
</table>
<div>
<div>
<a className="lin" href="#" data-toggle="modal" data-target="#exampleModal" ><svg id='Capa_2' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 59 59'>
<path d='M53.566,44.283l-9.552-4.776C42.78,38.89,42,37.628,42,36.248V32.5 c0.268-0.305,0.576-0.698,0.904-1.162c1.302-1.838,2.286-3.861,2.969-5.984C47.098,24.977,48,23.845,48,22.5v-4 c0-0.88-0.391-1.667-1-2.217V10.5c0,0,1.187-9-11-9c-12.188,0-11,9-11,9v5.783c-0.609,0.55-1,1.337-1,2.217v4 c0,1.054,0.554,1.981,1.383,2.517C26.382,29.369,29,32.5,29,32.5v3.655c0,1.333-0.728,2.56-1.899,3.198L18.18,44.22 C15.603,45.625,14,48.326,14,51.261V54.5h44v-3.043C58,48.419,56.283,45.642,53.566,44.283z'
fill='#8697cb' />
<path d='M18.18,44.22l5.946-3.243c-0.034-0.033-0.005-0.043,0.065-0.036l2.91-1.587 C28.272,38.715,29,37.489,29,36.155V32.5c0,0-1.062-1.275-2.092-3.323h0c0-0.001-0.001-0.002-0.001-0.003 c-0.135-0.268-0.268-0.551-0.399-0.844c-0.018-0.041-0.036-0.08-0.054-0.121c-0.133-0.303-0.263-0.616-0.386-0.944 c-0.008-0.021-0.015-0.044-0.023-0.065c-0.108-0.29-0.209-0.589-0.306-0.896c-0.026-0.084-0.052-0.167-0.077-0.251 c-0.101-0.338-0.196-0.682-0.278-1.038C24.554,24.481,24,23.554,24,22.5v-4c0-0.88,0.391-1.667,1-2.217v-5.648 C23.587,9.539,21.397,8.5,18,8.5c-9.563,0-10,8-10,8v4.995c-0.526,0.475-1,1.154-1,1.914v3.455c0,0.911,0.479,1.711,1.194,2.174 c0.863,3.758,3.76,6.463,3.76,6.463v3.157c0,1.151-0.629,2.211-1.64,2.762L3.61,45.622C1.385,46.836,0,49.168,0,51.703V54.5h14 v-3.239C14,48.326,15.603,45.625,18.18,44.22z'
fill='#556080' />
<circle cx='47' cy='45.5' r='12' fill='#71c386' />
<path d='M53,44.5h-5v-5c0-0.552-0.448-1-1-1s-1,0.448-1,1v5h-5c-0.552,0-1,0.448-1,1s0.448,1,1,1h5v5 c0,0.552,0.448,1,1,1s1-0.448,1-1v-5h5c0.552,0,1-0.448,1-1S53.552,44.5,53,44.5z'
fill='#fff' />
</svg>
</a>

</div>
  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">New Study Group:</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
          </button>
        </div>
      <div class="modal-body">

        <SettingsPane items={menu} index="/settings/general" >
            <SettingsPage handler="/settings/general">
              <fieldset className="form-group">
                <label for="profileName">Study Name: </label>
                  <input type="text" className="form-control"  placeholder="Name"   />
              </fieldset>
                <fieldset className="form-group">
                  <label for="profileColor">Add Friends: </label>
                    <select  className="form-control" >
                      <option value="blue">Bob Marley</option>
                      <option value="green">Jen Aniston</option>
                      <option value="red">Mrs. Claus</option>
                      <option value="grey">Larry the Bird</option>
                    </select>

                      </fieldset>
              </SettingsPage>
        </SettingsPane>

      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal" onClick={this.cancelClick}>Cancel</button>
        <button type="button" class="btn btn-info" onClick={this.myClick}>Create</button>
      </div>
    </div>
  </div>
  </div>
</div>

      </div>

    );
  }
  myClick(){
    alert("Created New Study Group!");
  }
}export default Friends;
